import { gql } from 'apollo-angular';
import { Injectable } from '@angular/core';
import * as Apollo from 'apollo-angular';
export type Maybe<T> = T | null;
export type InputMaybe<T> = Maybe<T>;
export type Exact<T extends { [key: string]: unknown }> = { [K in keyof T]: T[K] };
export type MakeOptional<T, K extends keyof T> = Omit<T, K> & { [SubKey in K]?: Maybe<T[SubKey]> };
export type MakeMaybe<T, K extends keyof T> = Omit<T, K> & { [SubKey in K]: Maybe<T[SubKey]> };
export type MakeEmpty<T extends { [key: string]: unknown }, K extends keyof T> = { [_ in K]?: never };
export type Incremental<T> = T | { [P in keyof T]?: P extends ' $fragmentName' | '__typename' ? T[P] : never };
/** All built-in and custom scalars, mapped to their actual values */
export type Scalars = {
  ID: { input: string; output: string; }
  String: { input: string; output: string; }
  Boolean: { input: boolean; output: boolean; }
  Int: { input: number; output: number; }
  Float: { input: number; output: number; }
  DateTime: { input: any; output: any; }
};

export type Ingredient = {
  __typename?: 'Ingredient';
  count: Scalars['Int']['output'];
  id: Scalars['ID']['output'];
  measure: Scalars['String']['output'];
  name: Scalars['String']['output'];
  recipeId?: Maybe<Scalars['ID']['output']>;
  sequence?: Maybe<Scalars['Int']['output']>;
};

export type IngredientInput = {
  count: Scalars['Int']['input'];
  measure: Scalars['String']['input'];
  name: Scalars['String']['input'];
};

export type Mutation = {
  __typename?: 'Mutation';
  addIngredient?: Maybe<Recipe>;
  addStep?: Maybe<Recipe>;
  createRecipe?: Maybe<Recipe>;
  deleteIngredient?: Maybe<Recipe>;
  deleteRecipe?: Maybe<Recipe>;
  deleteStep?: Maybe<Recipe>;
  updateRecipe?: Maybe<Recipe>;
};


export type MutationAddIngredientArgs = {
  ingredientData?: InputMaybe<IngredientInput>;
  recipeId: Scalars['ID']['input'];
};


export type MutationAddStepArgs = {
  recipeId: Scalars['ID']['input'];
  stepData?: InputMaybe<StepInput>;
};


export type MutationCreateRecipeArgs = {
  recipeData?: InputMaybe<RecipeInput>;
};


export type MutationDeleteIngredientArgs = {
  data?: InputMaybe<RemoveIngredientInput>;
};


export type MutationDeleteRecipeArgs = {
  id: Scalars['ID']['input'];
};


export type MutationDeleteStepArgs = {
  data?: InputMaybe<RemoveStepInput>;
};


export type MutationUpdateRecipeArgs = {
  id: Scalars['ID']['input'];
  recipeData?: InputMaybe<RecipeInput>;
};

export type Query = {
  __typename?: 'Query';
  recipe?: Maybe<Recipe>;
  recipes: Array<Maybe<Recipe>>;
};


export type QueryRecipeArgs = {
  id: Scalars['ID']['input'];
};

export type Recipe = {
  __typename?: 'Recipe';
  createdAt: Scalars['DateTime']['output'];
  description: Scalars['String']['output'];
  id: Scalars['ID']['output'];
  ingredients?: Maybe<Array<Maybe<Ingredient>>>;
  name: Scalars['String']['output'];
  steps?: Maybe<Array<Maybe<Step>>>;
  updatedAt: Scalars['DateTime']['output'];
};

export type RecipeInput = {
  description?: InputMaybe<Scalars['String']['input']>;
  ingredients?: InputMaybe<Array<InputMaybe<IngredientInput>>>;
  name?: InputMaybe<Scalars['String']['input']>;
  steps?: InputMaybe<Array<InputMaybe<StepInput>>>;
};

export type RemoveIngredientInput = {
  ingredientId?: InputMaybe<Scalars['ID']['input']>;
  recipeId?: InputMaybe<Scalars['ID']['input']>;
};

export type RemoveStepInput = {
  recipeId?: InputMaybe<Scalars['ID']['input']>;
  stepId?: InputMaybe<Scalars['ID']['input']>;
};

export type Step = {
  __typename?: 'Step';
  description: Scalars['String']['output'];
  id: Scalars['ID']['output'];
  image?: Maybe<Scalars['String']['output']>;
  recipeId?: Maybe<Scalars['ID']['output']>;
  sequence?: Maybe<Scalars['Int']['output']>;
  title?: Maybe<Scalars['String']['output']>;
};

export type StepInput = {
  description: Scalars['String']['input'];
  image?: InputMaybe<Scalars['String']['input']>;
  title?: InputMaybe<Scalars['String']['input']>;
};

export type AllRecipesQueryVariables = Exact<{ [key: string]: never; }>;


export type AllRecipesQuery = { __typename?: 'Query', recipes: Array<{ __typename?: 'Recipe', id: string, name: string, description: string, createdAt: any, updatedAt: any } | null> };

export type RecipeQueryVariables = Exact<{
  id: Scalars['ID']['input'];
}>;


export type RecipeQuery = { __typename?: 'Query', recipe?: { __typename?: 'Recipe', id: string, name: string, description: string, createdAt: any, updatedAt: any, ingredients?: Array<{ __typename?: 'Ingredient', id: string, name: string, count: number, measure: string } | null> | null, steps?: Array<{ __typename?: 'Step', id: string, description: string, title?: string | null, image?: string | null } | null> | null } | null };

export type CreateRecipeMutationVariables = Exact<{
  recipeData: RecipeInput;
}>;


export type CreateRecipeMutation = { __typename?: 'Mutation', createRecipe?: { __typename?: 'Recipe', id: string, name: string, description: string, createdAt: any, updatedAt: any } | null };

export type DeleteRecipeMutationVariables = Exact<{
  id: Scalars['ID']['input'];
}>;


export type DeleteRecipeMutation = { __typename?: 'Mutation', deleteRecipe?: { __typename?: 'Recipe', id: string, name: string, description: string } | null };

export type UpdateRecipeMutationVariables = Exact<{
  id: Scalars['ID']['input'];
  recipeData: RecipeInput;
}>;


export type UpdateRecipeMutation = { __typename?: 'Mutation', updateRecipe?: { __typename?: 'Recipe', id: string, name: string, description: string } | null };

export type AddStepMutationVariables = Exact<{
  recipeId: Scalars['ID']['input'];
  stepData: StepInput;
}>;


export type AddStepMutation = { __typename?: 'Mutation', addStep?: { __typename?: 'Recipe', id: string, name: string, description: string, createdAt: any, updatedAt: any, steps?: Array<{ __typename?: 'Step', id: string, description: string, title?: string | null, image?: string | null } | null> | null, ingredients?: Array<{ __typename?: 'Ingredient', id: string, name: string, count: number, measure: string } | null> | null } | null };

export type AddIngredientMutationVariables = Exact<{
  recipeId: Scalars['ID']['input'];
  ingredientData: IngredientInput;
}>;


export type AddIngredientMutation = { __typename?: 'Mutation', addIngredient?: { __typename?: 'Recipe', id: string, name: string, description: string, createdAt: any, updatedAt: any, steps?: Array<{ __typename?: 'Step', id: string, description: string, title?: string | null, image?: string | null } | null> | null, ingredients?: Array<{ __typename?: 'Ingredient', id: string, name: string, count: number, measure: string } | null> | null } | null };

export const AllRecipesDocument = gql`
    query allRecipes {
  recipes {
    id
    name
    description
    createdAt
    updatedAt
  }
}
    `;

  @Injectable({
    providedIn: 'root'
  })
  export class AllRecipesGQL extends Apollo.Query<AllRecipesQuery, AllRecipesQueryVariables> {
    document = AllRecipesDocument;
    
    constructor(apollo: Apollo.Apollo) {
      super(apollo);
    }
  }
export const RecipeDocument = gql`
    query recipe($id: ID!) {
  recipe(id: $id) {
    id
    name
    description
    ingredients {
      id
      name
      count
      measure
    }
    steps {
      id
      description
      title
      image
    }
    createdAt
    updatedAt
  }
}
    `;

  @Injectable({
    providedIn: 'root'
  })
  export class RecipeGQL extends Apollo.Query<RecipeQuery, RecipeQueryVariables> {
    document = RecipeDocument;
    
    constructor(apollo: Apollo.Apollo) {
      super(apollo);
    }
  }
export const CreateRecipeDocument = gql`
    mutation CreateRecipe($recipeData: RecipeInput!) {
  createRecipe(recipeData: $recipeData) {
    id
    name
    description
    createdAt
    updatedAt
  }
}
    `;

  @Injectable({
    providedIn: 'root'
  })
  export class CreateRecipeGQL extends Apollo.Mutation<CreateRecipeMutation, CreateRecipeMutationVariables> {
    document = CreateRecipeDocument;
    
    constructor(apollo: Apollo.Apollo) {
      super(apollo);
    }
  }
export const DeleteRecipeDocument = gql`
    mutation DeleteRecipe($id: ID!) {
  deleteRecipe(id: $id) {
    id
    name
    description
  }
}
    `;

  @Injectable({
    providedIn: 'root'
  })
  export class DeleteRecipeGQL extends Apollo.Mutation<DeleteRecipeMutation, DeleteRecipeMutationVariables> {
    document = DeleteRecipeDocument;
    
    constructor(apollo: Apollo.Apollo) {
      super(apollo);
    }
  }
export const UpdateRecipeDocument = gql`
    mutation UpdateRecipe($id: ID!, $recipeData: RecipeInput!) {
  updateRecipe(id: $id, recipeData: $recipeData) {
    id
    name
    description
  }
}
    `;

  @Injectable({
    providedIn: 'root'
  })
  export class UpdateRecipeGQL extends Apollo.Mutation<UpdateRecipeMutation, UpdateRecipeMutationVariables> {
    document = UpdateRecipeDocument;
    
    constructor(apollo: Apollo.Apollo) {
      super(apollo);
    }
  }
export const AddStepDocument = gql`
    mutation AddStep($recipeId: ID!, $stepData: StepInput!) {
  addStep(recipeId: $recipeId, stepData: $stepData) {
    id
    name
    description
    steps {
      id
      description
      title
      image
    }
    ingredients {
      id
      name
      count
      measure
    }
    createdAt
    updatedAt
  }
}
    `;

  @Injectable({
    providedIn: 'root'
  })
  export class AddStepGQL extends Apollo.Mutation<AddStepMutation, AddStepMutationVariables> {
    document = AddStepDocument;
    
    constructor(apollo: Apollo.Apollo) {
      super(apollo);
    }
  }
export const AddIngredientDocument = gql`
    mutation AddIngredient($recipeId: ID!, $ingredientData: IngredientInput!) {
  addIngredient(recipeId: $recipeId, ingredientData: $ingredientData) {
    id
    name
    description
    steps {
      id
      description
      title
      image
    }
    ingredients {
      id
      name
      count
      measure
    }
    createdAt
    updatedAt
  }
}
    `;

  @Injectable({
    providedIn: 'root'
  })
  export class AddIngredientGQL extends Apollo.Mutation<AddIngredientMutation, AddIngredientMutationVariables> {
    document = AddIngredientDocument;
    
    constructor(apollo: Apollo.Apollo) {
      super(apollo);
    }
  }